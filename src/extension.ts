...
function acceptancePrompt(receipt: CandidateReceipt): string {
  return [
    ...
    "Call herdr_capture_native_evidence once. It independently captures current Git content and returns tests and reviews records only when this live Pi process observed successful native test commands and a structured no-blocker foreground review on that unchanged code state.",
    "Call herdr_accept_candidate exactly once with only the attempt ID and candidate digest. The tool resolves the exact producer-owned tests and reviews index for the captured worker session and rejects missing or mismatched records. If capture reports missing, stale, failed, or ambiguous evidence, report that acceptance is blocked and do not call herdr_accept_candidate.",
